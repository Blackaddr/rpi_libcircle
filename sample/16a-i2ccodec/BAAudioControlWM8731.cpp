...
void BAAudioControlWM8731::enable(void)
{
	...
	write(WM8731_REG_INTERFACE, 0x42); // I2S, 16 bit, MCLK slave
...
bool BAAudioControlWM8731::write(unsigned int reg, unsigned int val)
{
	...
	while (!done) {
		...
		buffer[1] = val & 0xFFU;
		int result = m_i2cMaster.Write (WM8731_I2C_ADDR, buffer, 2);
		if (result != 2) {
			// retry
			CString msg;
			msg.Format("BAAudioControlWM8731::write(): could not write 2 bytes, only wrote %d\n", result);
